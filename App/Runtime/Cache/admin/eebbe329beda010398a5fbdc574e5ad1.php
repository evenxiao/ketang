...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
		...
		<a href="javascript:;" onclick="member_add('添加版块','<?php echo U('Sys/addCate');?>','','400')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加版块</a></span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<tbody>
			<?php if(is_array($data["cateList"])): $i = 0; $__LIST__ = $data["cateList"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				<td><input type="checkbox" value="<?php echo ($vo["id"]); ?>" name=""></td>
				<td><?php echo ($vo["id"]); ?></td>
				<td><?php echo ($vo["name"]); ?></td>
				<td><?php if($vo["type"] == 1): ?>视频<?php else: ?>资讯<?php endif; ?></td>
				...
				<td class="td-manage">
					...
					<a style="text-decoration:none" class="ml-5" onClick="member_edit('编辑','<?php echo U('Sys/updateCate');?>','<?php echo ($vo["id"]); ?>','800','400')" href="javascript:;" title="编辑">编辑</a>
				</td>
